...
::: tip Quant Link
期权价格 $C(S, \sigma, r, t)$ 是多元函数——标的价格、波动率、利率、剩余时间。期权希腊值本质上就是偏导数：
...
::: tip Quant Link
梯度下降是**所有**机器学习模型训练的基石。线性回归、逻辑回归、神经网络，训练过程就是：
...
- $H$ 所有特征值 > 0 → 函数在该点是**凸的**（局部最小值）
- $H$ 所有特征值 < 0 → 函数在该点是**凹的**（局部最大值）
- $H$ 特征值有正有负 → **鞍点**（一个方向凸、另一个方向凹）
