...
本章直接依赖 **01 高等数学**（梯度是偏导数的向量，Hessian 矩阵是二阶偏导数的矩阵）和 **02 线性代数**（组合方差 $\boldsymbol{w}^\top\Sigma\boldsymbol{w}$ 是二次型，协方差矩阵的特征分解决定优化曲面的曲率）。没有前两章的铺垫，无法理解梯度下降的方向、拉格朗日乘数法中的线性方程组、以及凸函数 Hessian 半正定的含义。
