...
2.1 向量与矩阵
    └── 数据的基本组织形式：组合权重向量、收益率矩阵
        ...
        ▼
2.2 线性方程组
    └── 因子模型的求解：$X\beta = y$ 的最小二乘解
        │
        ▼
2.3 正定矩阵与二次型
    └── 组合方差 $\boldsymbol{w}^T\Sigma\boldsymbol{w}$ 永远不会为负——因为协方差矩阵是正定的
        │
        ▼
2.4 特征值分解 & SVD
...
**前置知识**：
- [01 高等数学](/prerequisite-math/01-calculus/)（尤其是 1.2 导数的概念，为 2.6 矩阵微积分做准备）
- 基本代数运算
...
**跳过建议**：如果主要关注因子模型和组合优化，2.5 和 2.6 是最重要的应用节，不可跳过。如果时间紧迫，2.4 的理论部分可以略读，直接看 2.5 的应用。

> **下一步**：掌握了多维数据处理方法后，进入 [03 概率论](/prerequisite-math/03-probability/)——为不确定性建立数学模型。
